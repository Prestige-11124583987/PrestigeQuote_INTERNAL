...
assert.equal(STANDARD_DOOR_ROW_HEIGHT, 72);
assert.equal(normalizeDoorRowHeight(38), 72);
assert.equal(normalizeDoorRowHeight(58), 72);
assert.equal(normalizeDoorRowHeight(71.2), 72);
assert.equal(normalizeDoorRowHeight(72), 72);
assert.equal(normalizeDoorRowHeight(72.1), 73);
assert.equal(normalizeDoorRowHeight(96.2), 97);

const fiveOrdinary = rows(72, 72, 72, 72, 72);
let pages = paginateIndivisibleRows(fiveOrdinary);
assert.equal(pages.length, 1);
assert.deepEqual(pages[0].rows.map((row) => row.id), [1, 2, 3, 4, 5]);

const sixOrdinary = rows(72, 72, 72, 72, 72, 72);
...
const oversizedDoor = rows(600, 72);
