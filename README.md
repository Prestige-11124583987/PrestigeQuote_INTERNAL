...
The approved combined `01-Door-Order-Process-and-Product-Warranty.pdf` ships in `invoice-supplements/` and appends behind every Quote for every salesperson. See `SUPPLEMENT-GUIDE.md`.
...
- Every add-on has an **On / Off** control in Pricing & Options. Turning an add-on off hides it from new quotes without deleting its name or pricing.
...
Edit the root-level `EDIT-PRICING-HERE.json` file to change default prices, discounts, add-on names, add-on visibility, installation pricing, and dropdown options without editing application code.
